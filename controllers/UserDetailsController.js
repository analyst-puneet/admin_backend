...
const Create = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Error parsing eduDetails:", e);
      eduDetails = [];
    }
    ...
    for (const edu of eduDetails) {
      ...
      const educationDocs = eduDetails
        .filter((edu) => edu && educationTypeMap[edu.qualification])
        .map((edu) => {
          const doc = {
            user_id: String(userRecord.sequenceNo),
            education_type_id: educationTypeMap[edu.qualification],
