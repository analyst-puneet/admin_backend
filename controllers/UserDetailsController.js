  ...
  try {
    ...
    if (req.files) {
      ...
      filesArray.forEach((file) => {
        ...
        else if (file.fieldname === 'joiningLetter') documentFields.joiningLetter = filePath;
        else if (file.fieldname === 'offerLetter') documentFields.offerLetter = filePath;
        else if (file.fieldname === 'aadharFront') documentFields.aadharFront = filePath;
        else if (file.fieldname === 'aadharBack') documentFields.aadharBack = filePath;
        else if (file.fieldname === 'panCard') documentFields.panCard = filePath;
        
        // Handle education document uploads
        else if (file.fieldname === 'tenthMarksheet') documentFields.tenthMarksheet = filePath;
        else if (file.fieldname === 'twelfthMarksheet') documentFields.twelfthMarksheet = filePath;
        else if (file.fieldname === 'phdCertificate') documentFields.phdCertificate = filePath;
        else if (file.fieldname === 'tenthCertificate') documentFields.tenthCertificate = filePath;
        else if (file.fieldname === 'twelfthCertificate') documentFields.twelfthCertificate = filePath;
    ...
    let eduDetails = [];
    try {
      if (bodyData.eduDetails) {
        // First try to parse as JSON if it's a string
        const parsedDetails = typeof bodyData.eduDetails === 'string' 
          ? JSON.parse(bodyData.eduDetails) 
          : bodyData.eduDetails;
        
        // Ensure it's always an array
        eduDetails = Array.isArray(parsedDetails) ? parsedDetails : [parsedDetails];
        
        // Filter out any null/undefined entries
        eduDetails = eduDetails.filter(edu => edu && edu.qualification);
      }
    } catch (e) {
      ...
      try {
        ...
        switch (edu.qualification) {
          case '10th':
            ...
            if (documentFields.tenthMarksheet) {
              educationDoc.certificate.push({
                type: 'marksheet',
                path: documentFields.tenthMarksheet
              });
            }
            if (documentFields.tenthCertificate) {
              educationDoc.certificate.push({
                type: 'certificate',
          ...
          case '12th':
            ...
            if (documentFields.twelfthMarksheet) {
              educationDoc.certificate.push({
                type: 'marksheet',
                path: documentFields.twelfthMarksheet
              });
            }
            if (documentFields.twelfthCertificate) {
              educationDoc.certificate.push({
                type: 'certificate',
          ...
          case 'UG':
            ...
            educationDoc.marksheet = documentFields.ugMarksheets.filter(Boolean).join(',') || '';
          ...
          case 'PG':
            educationDoc.course = edu.course || '';
            educationDoc.duration = edu.duration || 2;
            educationDoc.marksheet = documentFields.pgMarksheets.filter(Boolean).join(',') || '';
    ...
    // 11. Save education qualifications with transaction support
